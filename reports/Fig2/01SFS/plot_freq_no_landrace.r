...
b=4
 ...
 # barplot(r,beside = T,ylim=c(0,0.4),col=col,border = col,xlab="Derived allele frequency",ylab="Proportion of sites",las=1)
  ...
  par(new=T)
  barplot(r2,beside = T,ylim=c(0,0.5),col="black",border = "black",angle=45,density = 10,las=1,lwd=0.1) 
...
for(b in c(1:6)[-4])
{
  ...
  par(new=T)
  barplot(r2,beside = T,ylim=c(0,max(r)),col="black",border = "black",angle=45,density = 10,las=1,lwd=0.1) 
  axis(1,at=c(0,x[seq(4,40,by=4)]+0.4),labels = seq(0,1,by=0.1))
...
col=c("#E39500","#43A178")
##################
d_TEO=fread("TEO_Concatenated_delterious_allele_count.txt", header=T,data.table=F)[,-3]
d_MZ=fread("282panel_Concatenated_delterious_allele_count.txt", header=T,data.table=F)[,-3]
d_freq_T=d_TEO[,3]/d_TEO[,4]
d_freq_M=d_MZ[,3]/d_MZ[,4]
d_TEO1=cbind(d_TEO,d_freq_T)
d_MZ1=cbind(d_MZ,d_freq_M)
...
###plot
  ...
  ###########plot deleterious allele SFS
  d_TEO3=hist(d_TEO1[,5],breaks = 20)
  d_MZ3=hist(d_MZ1[,5],breaks = 20)
  d_TEO_f=d_TEO3$counts/nrow(TEO1)
  d_MZ_f=d_MZ3$counts/nrow(MZ1)
  dd2=cbind(d_TEO_f,d_MZ_f)
  colnames(dd2)=NULL
  r2=NULL
  for(i in 1:nrow(dd2))
