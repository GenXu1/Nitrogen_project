...
d1=fread("Glu.CHG_Teo_Maize_input.txt", header=T,data.table=F,na.strin="NA")
d1=d1[,c(1:2,23:36)]
...
write.table(res,file="CHG_DNA-meth_exon.txt",row.names = F,quote = F,sep="\t")

d=fread("CHG_DNA-meth_exon.txt", header=T,data.table=F,na.strin="NA")
pdf("CHG_DNA-meth_exon.pdf",height = 2.7,width = 5.5)
