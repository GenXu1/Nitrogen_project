...
/common/jyanglab/gxu6/project/07_282panel/01GCTB

/common/jyanglab/gxu6/project/6_Yield_traits_N_stress/GCTB
...
chl <- fread("/common/jyanglab/gxu6/project/07_282panel/01GCTB/leaf_nutrient_traits/Results_HN_LN_Ratio/CHL.snpRes", header=TRUE, data.table=FALSE)

chl <- chl[order(abs(chl$A1Effect), decreasing = T),]
chl$rank <- 1:nrow(chl)
head(chl, 30)[, 1:9]
BP = 100000
ch5 <- subset(chl, Chrom == 5 & Position > 47394843 - BP & Position < 48461304 + BP)
...
for(i in idx){
  ...
  tem <- data.frame(trait= names(d)[i], pval=pval)
  out <- rbind(out, tem)
}
