...
d=fread("LR.AGPV4.B0_MAF.stat.txt", header=T,data.table=F)
d=d[,c(1,2,4)]
thr1=quantile(d[,3],0.99)
thr2=quantile(d[,3],0.95)
sig=d[d[,3]>=thr1,]
sig2=d[d[,3]>=thr2,]
write.table(sig,file="Sig99.LR.AGPV4.B0_MAF.stat.txt",col.names = T,row.names =F,quote=F,sep="\t")
write.table(sig2,file="Sig95.LR.AGPV4.B0_MAF.stat.txt",col.names = T,row.names =F,quote=F,sep="\t")
...
dd$physPos=dd$physPos/1e6
  ...
  png("LR.AGPV4.B0_MAF.png",height = 150,width =210,res=600,units = "mm")
